def choose_action(contakt_list):
    while True:
...
3 - Изменить контакт\n4 - Удалить контакт\n5 - Просмотреть все контакты\n0 - Выйти из приложения\n')
        print()
        if user_choice == '1':
            contact_list = read_file_to_dict(contakt_list)
            find_number(contact_list)
        elif user_choice == '2':
            add_phone_number(contakt_list)
        elif user_choice == '3':
            change_phone_number(contakt_list)
        elif user_choice == '4':
            delete_contact(contakt_list)
        elif user_choice == '5':
            show_phonebook(contakt_list)
        elif user_choice == '0':
            print('До свидания!')
            break
        else:
